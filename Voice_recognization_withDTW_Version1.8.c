...
#define THRESHOLD 1500000 //Sensitivity of the microphone 2000000
...
int16_t scout_squad[3]; //(int16_t for those three array)
...
while (1)
{
	if (signal == 0)
	{
		if (inst != 6)
		{
		  printf("*********VOICE RECOGNITION MACHINE CEREAL VERSION 1.8**************\n");
		...
		else
		{
			if (iterator == full_size) //&& memoryLoad == 1) // if it did not detect the start of the input, i wont't increment to 1024
			{
				...
				if (inst == 6)
				{
					...
					DTW_initialization();
					DTW_process();
					match_lib = least_cost();

					char *temp_vocab;
					temp_vocab = (char *)malloc(10*sizeof(char));
					printf("%s \n", Library[match_lib].vocab);
					temp_vocab = " ";
					temp_vocab = Library[match_lib].vocab;
