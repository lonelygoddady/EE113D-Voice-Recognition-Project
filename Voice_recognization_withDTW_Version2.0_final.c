...
struct Data Library[data_base_size];  //the Library 
		...
		if (inst == 1 || inst == 3 || inst == 6)
		{
			if (inst == 6 || (inst == 1 && admin ==0))
			{
				if (free_space == data_base_size)
				{
					library_initiation();
					if (memoryLoad == 0)
						memory_load(); //load the free_mode Library
				}
			}

			signal = 1; //tell the interrupt to start recording

			if (inst != 3 && free_space == data_base_size)
		...
		else
		{
			if (iterator == full_size) //&& memoryLoad == 1) // if it did not detect the start of the input, i wont't increment to 1024
			{
				...
				if (inst == 1)
				{
					...
					{
						if (strcmp(Library[match_index].name, "ben") == 0 || strcmp(Library[match_index].name, "Ben") == 0)
						{
							if (curr_pos == 0)
							{
				...
				if (inst == 6)
				{
					...
					sentence[current_vocab] = (char*)malloc(sentence_length*sizeof(char));
					...
					if (strcmp(temp_vocab, "Finished") == 0)
					{
					...
					{
						initialization();  //basically the reboot_inquiry except the inst = 0
						syc_initiation();
						iterator = 0;
						check = 0;
						signal = 0;
				...
				for (p = 0;p < frame_num;p++)
				{
					for (i = 0; i< MFCC_size; i++) //y- avg
					{
						for (j = 0;j<MFCC_size; j++) //x - new
						{
							...
							// record the difference squared between MFCC processed input voice (MFCC_TOTAL[p][j]) and library reference (Library[k].MFCC_info[p][i])
						}
					}
				}
			}
				...
				for (p = 0; p < frame_num; p++)
				{
					...
					for (i = 1; i<MFCC_size;i++)
					{
						for (j = 1; j<MFCC_size;j++)
						{
							...
							DTW_match[0] = DTW[k][p][i-1][j-1] + gap;
							DTW_match[1] = DTW[k][p][i][j-1] + gap;
							DTW_match[2] = DTW[k][p][i-1][j] + gap;
...
float set_small(float x, float y, float z) // gets the three values from DTW matrix and returns the least value with var
{
	float match = x;
...
int low_dev() // after generating matrices with MFCC for DTW calculation format, and find the least distance by comparing
{
			...
			for (k = 0; k<data_base_size; k++)
			{
				...
				for (p = 0; p< frame_num;p++)
				{
					...
					while (y>0 || x>0)
					{
						...
						trace[k][p][traces][0] = x; //
						trace[k][p][traces][1] = y; //
					}
					traces++;
					trace[k][p][traces][0] = 0; //may not need this if we initialize the matrix
					...
					{
						trace_cost[k] += match_result[k][p][(trace[k][p][j][1])][(trace[k][p][j][0])]; //initializtize this array
					}
				}
			}
			int min_index = 0;
...
void phase_match()
{
	...
	for (j = 0; j< sentence_length; j++) //loop through the sentence 
	{
		...
		if (strcmp(sentence[j],"on") == 0 && match_count == 1)
		{
			match_count = 2;
		}
		if (strcmp(sentence[j],"off") == 0 && match_count == 1)
		{
			match_count = 3;
		}

		if (strcmp(sentence[j],"LED") == 0 && match_count == 2 )
		{
			match_count = 4;
		}
		if (strcmp(sentence[j],"LED") == 0 && match_count == 3 )
		{
			match_count = 5;
		}
		...
		if (strcmp(sentence[j],"Library") == 0 && match_count == 6 )
		{
			match_count = 7;
		}
