...
#define data_base_size 11 //size of the library
...

{
  ...
  while (1)
  {
	if (signal == 0)
	{
		  printf("*********VOICE RECOGNITION MACHINE CEREAL VERSION 1.1**************\n");
...
void file_name_load() //get all the file names
{
	...
	file_name[5] = (char *)malloc(8*sizeof(char));
	file_name[5] = "C:\\Users\\EE113D\\MyProjects\\voice_recongization_project\\memory\\memory_6.txt";
	file_name[6] = (char *)malloc(8*sizeof(char));
	file_name[6] = "C:\\Users\\EE113D\\MyProjects\\voice_recongization_project\\memory\\memory_7.txt";
	file_name[7] = (char *)malloc(8*sizeof(char));
	file_name[7] = "C:\\Users\\EE113D\\MyProjects\\voice_recongization_project\\memory\\memory_8.txt";
	file_name[8] = (char *)malloc(8*sizeof(char));
	file_name[8] = "C:\\Users\\EE113D\\MyProjects\\voice_recongization_project\\memory\\memory_9.txt";
	file_name[9] = (char *)malloc(8*sizeof(char));
	file_name[9] = "C:\\Users\\EE113D\\MyProjects\\voice_recongization_project\\memory\\memory_10.txt";
	file_name[10] = (char *)malloc(8*sizeof(char));
	file_name[10] = "C:\\Users\\EE113D\\MyProjects\\voice_recongization_project\\memory\\memory_11.txt";
}
...
/*void input_eval ()
{
  if (!ready) // if incorrect
	{
				printf("What is the correct word? ");
				//scanf("%d", &correct);
				scanf(" %s", &c_correct);

				for (i = 0; i< NUM_TEST_CHAR; i++)
				{
					if ((char)c_correct == Library[i].vocab)
					{
						correct = i;
					}
				}

				if (switch_8_on && (correct != NUM_TEST_CHAR-1))
				{
					num_wrong[correct]++;
				}
            }
	...
	record_trials[correct][0]++;



	if (correct != NUM_TEST_CHAR-1)
	{
		for (p = 0; p< num_window;p++)
		{
			for (i = 0; i<path_lengths_records[correct][p];i++)
			{
new_mfcc_index = path[correct][p][i][0];
avg_mfcc_index = path[correct][p][i][1];
MFCC_avg[correct][p][avg_mfcc_index] = (float)((MFCC_avg[correct][p][avg_mfcc_index]*(n_trial-1) + WMFCC_new[p][new_mfcc_index]))/n_trial;
			}
		}
	}
	else
