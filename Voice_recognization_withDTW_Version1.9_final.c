...
#define data_base_size 69 //size of the library (60) - six ppl: George, Ben, Lyn, Briggs, Yuyu, Brooke
...
int admin = 0; //indicate the status of the user (admin or not)
...
while (1)
{
	if (signal == 0)
	{
		if (inst != 6)
		{
		  printf("*********VOICE RECOGNITION MACHINE CEREAL VERSION 1.9**************\n");
		  if (admin == 1)
		  {
		...
		else
		{
			if (iterator == full_size) //&& memoryLoad == 1) // if it did not detect the start of the input, i wont't increment to 1024
			{
				...
				if (inst == 1)
				{
					...
					{
						if (strcmp(Library[match_index - 1].name, "ben") == 0 || strcmp(Library[match_index - 1].name, "Ben") == 0)
						{
							if (curr_pos == 0)
							{
								strcmp(Library[match_index - 1].vocab, "cat" == 0 || strcmp(Library[match_index - 1].vocab, "Cat" == 0)
								{
									successful_trial++;
									curr_pos++;
								}
							...
							if (curr_pos == 1)
							{
								strcmp(Library[match_index - 1].vocab, "no" == 0 || strcmp(Library[match_index - 1].vocab, "No" == 0)
								{
									successful_trial++;
									curr_pos++;
								}
							...
							if (curr_pos == 2)
							{
								strcmp(Library[match_index - 1].vocab, "crazy" == 0 || strcmp(Library[match_index - 1].vocab, "Crazy" == 0)
								{
									successful_trial++;
									curr_pos++;
								}
						...
						if (successful_r > 80)
						{
							...
							printf("Admin status detected!\n");
						}
						if (remain_trial == 4 && admin == 0)
						{
							Total_trial = 0;
							printf("Admin not detected! Start it over!\n");
						}
					}
